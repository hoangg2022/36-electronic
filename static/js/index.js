$(document).ready(function() {
    ...
    // Fetch products for products.html
    ...
    window.addToCart = function(productId, productName, productPrice) {
        ...
        $('#cartQuantity').on('input', function() {
            ...
            if (quantity < 1) $(this).val(1); // Enforce minimum quantity
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data.forEach(category => {
                    select.append(`<option value="/products?category_id=${category.id}" style="background: #fff; color: #4a90e2;">${category.name}</option>`);
                });
            },
            error: function() {
                $('#categorySelect').append('<option style="background: #fff; color: #e63946;">Lỗi tải danh mục</option>');
            }
        });
    }
